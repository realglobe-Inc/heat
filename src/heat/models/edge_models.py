...
class HeatEdge(nn.Module):
    ...
    def forward(
        ...
        edge_feats = edge_feats.view(
            bs, num_edges, edge_feats.shape[2] * edge_feats.shape[3]
        )

        edge_inputs = self.edge_input_fc(
            edge_feats.view(bs * num_edges, edge_feats.shape[2])
        )
        edge_inputs = edge_inputs.view(bs, num_edges, edge_feats.shape[2])

        edge_center = (
            edge_coords[:, :, 0, :].float() + edge_coords[:, :, 1, :].float()
        ) / 2
...
class EdgeTransformer(nn.Module):
    ...
    def forward(
        ...
        hs, inter_references = self.relational_decoder(
            hybrid_prim_hs,
            filtered_rp,
            memory,
            spatial_shapes,
        ...
        hs_rel, _ = self.relational_decoder(
            rel_prim_hs,
            filtered_rp,
            memory,
            spatial_shapes,
