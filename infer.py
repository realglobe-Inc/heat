...
def get_results(
    ...
    for tt in range(infer_times):
        ...
        s1_predicates = s1_logits.squeeze(0).softmax(0)
        s2_predicates_rel = s2_logits_rel.squeeze(0).softmax(0)
        s2_predicates_hb = s2_logits_hb.squeeze(0).softmax(0)
        s1_predicates_np = s1_predicates[1, :].detach().cpu().numpy()
        s2_predicates_rel_np = s2_predicates_rel[1, :].detach().cpu().numpy()
        ...
        if tt != infer_times - 1:
            ...
            pos_edge_ids = np.where(s2_predicates_np >= 0.9)
            ...
            pos_edge_ids = np.where(s2_predicates_np >= 0.5)
            for pos_id in pos_edge_ids[0]:
                actual_id = selected_ids[pos_id]
